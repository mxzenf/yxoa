<#include "../pageTitleBread.ftl">
	<div id="page-content">
		...
		<div class="col-sm-10 eq-box-sm">
			<div class="panel lion-fullHeight">
				<div class="panel-body">
					<a id="add" class="btn btn-info data-toolbar" onclick="doPjax('${Request.ctx}/admin/org/getEditPage.shtml')" href="javascript:void(0);"><i class="demo-pli-add"></i> 添加</a>
...
<script>
	...
	var col = [
			...
			},
			{
				...
				formatter : function(value, row, index) {
					...
					//value：当前field的值，即id
					//row：当前行的数据
					var a = '<a class="label label-table label-info" onclick="doPjax(\''+ctx+'/admin/org/getEditPage.shtml?parentid='+row.id+'\')" href="javascript:void(0);" >添加下级机构</a>&nbsp;';
					a = a + '<a class="label label-table label-success" onclick="doPjax(\''+ctx+'/admin/org/getEditPage.shtml?id='+row.id+'\')" href="javascript:void(0);" >编辑</a>&nbsp;';
	...
	var opt = {
		...
		url : ctx + "/admin/org/listData.shtml",
	...
	function del(ids){
		pointLion.confimMsg("您是否要删除所选信息 ?<br/> 点击确认进行删除 , 点击取消取消该操作",null,function(){
			//执行删除
			var data = common_ajax.ajaxFunc("/admin/org/delete.shtml",{"ids":ids}, "json", null);
			//删除结果
			if(data.success){
