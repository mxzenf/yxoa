			        ...
			        <div class="col-sm-12 eq-box-sm">
			        	<div class="panel lion-fullHeight">
						    <div class="panel-body">
						    	<a id="add" class="btn btn-info data-toolbar" onclick="doPjax('${Request.ctx}/admin/notice/getEditPage.shtml')" href="javascript:void(0);"><i class="demo-pli-add"></i> 起草</a>
...
<script>
	var col = [{
	       ...
	       width:'250',
	       formatter:function(value,row,index){
	    	   var a  =     '<a class="label label-table label-success" onclick="doPjax(\''+ctx+'/admin/notice/getEditPage.shtml?id='+row.id+'&view=detail\')" href="javascript:void(0);" >查看</a>&nbsp;';
	    	   if(value=='0'){//未发布
		           	a = a + '<a class="label label-table label-success" onclick="doPjax(\''+ctx+'/admin/notice/getEditPage.shtml?id='+row.id+'\')" href="javascript:void(0);" >修改</a>&nbsp;';
	...
	var opt = {
		    //请求方法
			   ...
			   url: ctx+"/admin/notice/getListData.shtml",
	...
	function del(ids){
		pointLion.confimMsg("您是否要删除所选公告 ?<br/> 点击确认进行删除 , 点击取消取消该操作",null,function(){
			//执行删除
			var data = common_ajax.ajaxFunc("/admin/notice/delete.shtml",{"ids":ids}, "json", null);
			//删除结果
			if(data.success){
	...
	function publish(id){
			pointLion.confimMsg("您将要发布该公告！<br/><span style='color:green;'>被通知人将会收到该通知！<br/>您将无法进行修改和删除！！！</span> <br/>点击确认进行提交 , 点击取消取消该操作",null,function(){
				var data = common_ajax.ajaxFunc("/admin/notice/publish.shtml",JSON.stringify({"id":id}), "json",null,"application/json");
				if(data.success){
					pointLion.alertMsg("发布成功!","success",null,function(){
						doPjax(ctx+'/admin/notice/getListPage.shtml');//跳转到列表页
					});
				}
			});
